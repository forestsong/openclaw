...
- [x] 增加 `verify-pack.json`，先支持 `exec / logs / report`
- [x] 增加 browser-backed evaluator pack
- [x] 补一份 manual role-scoped build walkthrough
...
- `src/agents/verify-report.test.ts`
...
- `verify-pack.json` 现在支持 `browser` check，并能记录 navigate/action/assert/screenshot evidence
- `/context detail` 现在会显示 browser evaluator 的 evidence path，而不是只剩原始 browser 错误
- role-scoped build loop 现在已经有一份可直接照着执行的手工 walkthrough，包含 artifact 样例、prompt 模板和 stop conditions

### P5 Phase 2：把 role-scoped build loop 变成可审计的 build rounds

这一阶段不是直接做自动 loop runner，而是先把多轮 build / eval 的状态、证据和结论做扎实。

- [x] 让 `build-report.json` 和 `eval-report.json` 变成 round-aware artifact
- [x] 在 `/context` 里显示 active build run、latest round 和 top blocking findings
- [x] 增加 `api` evaluator pack
- [x] 增加 `log-file / artifact` evaluator pack
- [x] 把 `eval-report.json` 强化成稳定的 blocking-finding contract
- [x] 把 manual walkthrough 扩展到 multi-round 操作

**当前新增证据**

- `src/agents/build-runs.ts` 现在会给 `build-report.json / eval-report.json` 写入稳定 round metadata，并在每个 build run 下额外保留 `round-0001.build-report.json`、`round-0001.eval-report.json` 这类归档快照
- `readBuildRunRoundState(...)` / `readBuildRunRoundStateFromRunDir(...)` 现在能直接解析 latest builder round、latest evaluator round 和当前 latest round
- `src/agents/build-runs.test.ts` 已覆盖 multi-round 存储、latest-round 解析，以及旧单轮 canonical artifact 的兼容读取
- `docs/exec-plans/role-scoped-build-loop.md` 和 role-scoped walkthrough 里的 artifact 样例已经更新成新 schema
- `src/auto-reply/reply/commands-context-report.ts` 现在会直接从 build-run artifact root 读取 active run、latest rounds、top blocking findings 和 recommended next role，并在 `/context list/detail/json` 里展示
- `src/agents/verify-pack.ts` 和 `src/agents/verify-pack.test.ts` 现在支持 `api`、`log-file`、`artifact-text`、`artifact-json` 四类 evaluator pack，并带结构化 `response/file` evidence
- `eval-report.json` 的 `blocking_findings` 现在是稳定对象合同，支持 `id / kind / summary / severity / category / evidence_refs`，并保留旧字符串数组的兼容读取
- multi-round walkthrough 现在补上了 round 2 builder/evaluator 样例，以及 `/context detail` 该怎么看 latest round / findings / next role

**验收信号**

- 一个 build run 能清楚表示第几轮 builder / evaluator 正在做什么
- `/context detail` 能直接看出最新一轮为什么失败、证据在哪、下一步谁来接手
- evaluator 不只会看 browser 和 exec，还能检查 API、log 和 artifact 证据
- 在没有 loop DSL 的前提下，多轮 build/eval 也能被人类和 agent 正常调试

**当前计划入口**

- `docs/exec-plans/role-scoped-build-loop-phase-2-backlog.md`
- `docs/exec-plans/role-scoped-build-loop.md`
